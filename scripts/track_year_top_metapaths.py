...
def main() -> None:
    ...
    ref = ref.sort_values(["control", "year", "go_id", "metapath_rank"])
    if not _is_all(args.top_n):
        ref = ref.groupby(["control", "year", "go_id"], group_keys=False).head(int(args.top_n))
        top_n_label = f"top{int(args.top_n)}"
    else:
        top_n_label = "topall"
    ref = ref[["control", "year", "go_id", "metapath", "metapath_rank"]].rename(
