...
def plot_top_metapaths_per_go(
    ...
    if "metapath" not in by_metapath_df.columns or "effect_size_z" not in by_metapath_df.columns:
    ...
    for idx, go_id in enumerate(go_ids):
        ...
        if sub.empty:
            ax.axis("off")
            continue
        sub = sub.sort_values("effect_size_z", ascending=False).head(top_n)
        ...
        y = np.arange(len(sub))
        ax.barh(y, sub["effect_size_z"].astype(float), color=colors, edgecolor="black", linewidth=0.3)
        ax.set_yticks(y)
